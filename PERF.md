...
8. **~~Navigation-LOD~~ / ~~per-tier grass cap~~ — REVERTED (owner preference).** An
   earlier round hid foliage/figures/particles while moving and thinned grass per
   tier. The owner rejected any visible tradeoff: hiding tree leaves during motion
   exposed bare branches that read as "trees through buildings," and thinning grass
   is "deleting" detail. Both were fully removed — the scene now renders **complete
   and unchanged whether still or moving, on every tier**. The only movement lever
   that touches nothing geometric is the DPR interaction-boost (below).

Interaction-boost parameters: `boostScale 0.75`, `boostFloor 1.0`, `boostRestoreMs 250`
(temporary resolution softening during a gesture — removes no objects; restores on release).
